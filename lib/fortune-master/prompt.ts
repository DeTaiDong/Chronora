...
- 若不是拒答，answer 字段必须写 800-1200 个汉字，分成 5-8 个自然段，段落之间用两个换行分隔。内容要包含：直接结论、盘面依据、对用户问题的逐项回应、现实可执行建议、需要保留分寸的地方。不要为了凑字重复同一句话。
...
输出必须符合指定 JSON schema。若正常解读，answer 字段保持 800-1200 个汉字；若拒答，可简短说明。
`.trim();
...
4. 若正常作答，answer 必须有 800-1200 个汉字，分段完整，不要只给提纲。
`.trim();
}
